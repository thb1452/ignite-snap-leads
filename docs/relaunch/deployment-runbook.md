...
Use the native PostgreSQL workflow again for any changed SQL/harness and preserve the exact-commit artifact. Hosted tests additionally cover real managed Auth/PostgREST/gateway/realtime behavior. Actual Stripe sandbox tests currently have **zero completed provider executions**; the prepared harness alone is insufficient. Automatic review previously rejected new sandbox creation/owner-email disclosure, and CLI automatic provisioning was unavailable. Use an explicitly approved usable sandbox and securely supplied test credentials; do not retry through an indirect route to bypass that boundary.
