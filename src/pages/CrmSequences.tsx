...
const TRIGGER_LABELS: Record<string, string> = {
  ...
  distress_event: "Legacy event enrollment (retired)",
  stage_change: "Legacy stage enrollment (retired)",
};
    ...
    <AppLayout>
      ...
      <div className="flex items-start justify-between gap-4 mb-2">
        <PageHeader
          ...
          description="Prepare SMS sequences. Enrollment and sending remain held while connections are verified."
        />
        <NewSequenceDialog />
      </div>
          ...
          {!isLoading && sequences.length === 0 && (
            <Card>
              <CardContent className="py-8 text-center text-sm text-muted-foreground">
                No sequences yet. You can prepare a paused sequence for later review.
              </CardContent>
            </Card>
          )}
          {sequences.map((s) => (
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const { error } = await supabase.from("drip_sequences" as any).insert({
        ...
        trigger_type: "manual", trigger_config: {}, is_active: false,
      });
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">New sequences are saved paused. CRM changes never enroll contacts in messaging.</p>
        </div>
        <DialogFooter>
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0">
        ...
        <Button variant="outline" size="sm" onClick={toggleActive} disabled={!sequence.is_active}>
          {sequence.is_active ? <><PowerOff className="h-4 w-4 mr-1" />Pause</> : <><Power className="h-4 w-4 mr-1" />Activation held</>}
        </Button>
      </CardHeader>
