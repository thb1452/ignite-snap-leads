...
BEGIN
  ...
  IF p_sort_by = 'snap_score' THEN
    -- Sort by SnapScore descending, with total_violations as tiebreaker for tied scores
    ...
    FROM (
      ...
      WHERE (p_state IS NULL OR p.state ILIKE p_state)
        ...
        AND (NOT p_multiple_violations_only OR COALESCE(p.total_violations, 0) > 1)
        AND (NOT p_repeat_offender_only OR p.repeat_offender = true)
      ORDER BY p.snap_score DESC NULLS LAST, p.total_violations DESC NULLS LAST, p.id
      LIMIT p_page_size
      OFFSET v_offset
    ) props;
  ELSIF p_sort_by = 'newest_violation' THEN
    -- Sort by newest violation date
    SELECT jsonb_agg(row_to_json(props)::jsonb)
    INTO v_items
    FROM (
      SELECT 
  ...
  ELSE
    -- Default: recently_updated — sort by updated_at with snap_score + total_violations tiebreaker
    SELECT jsonb_agg(row_to_json(props)::jsonb)
    INTO v_items
    FROM (
      SELECT 
      ...
      WHERE (p_state IS NULL OR p.state ILIKE p_state)
        ...
        AND (NOT p_multiple_violations_only OR COALESCE(p.total_violations, 0) > 1)
        AND (NOT p_repeat_offender_only OR p.repeat_offender = true)
      ORDER BY p.updated_at DESC NULLS LAST, p.snap_score DESC NULLS LAST, p.total_violations DESC NULLS LAST, p.id
      LIMIT p_page_size
      OFFSET v_offset
    ) props;
  END IF;
