...
/**
 * Fetches property IDs that are missing snap_insight
 */
export async function getPropertiesMissingInsights(): Promise<string[]> {
  const allIds: string[] = [];
  const batchSize = 1000;
  let offset = 0;
  let hasMore = true;
  ...
  while (hasMore) {
    ...
    if (error) {
      console.error("Error fetching properties missing insights:", error);
      throw error;
    }
...
export async function countPropertiesMissingInsights(): Promise<number> {
  ...
  if (error) {
    console.error("Error counting properties missing insights:", error);
    throw error;
  }
...
/**
 * Generates insights for all properties that don't have one
 */
export async function batchGenerateMissingInsights(
  onProgress?: ProgressCallback
): Promise<{ success: boolean; processed: number; total: number }> {
  // Get only property IDs missing insights
  const propertyIds = await getPropertiesMissingInsights();
  ...
  try {
    for (let i = 0; i < propertyIds.length; i += BATCH_SIZE) {
      ...
      console.log(`[BatchInsights] Processing batch ${batchNumber}/${totalBatches} (${batch.length} properties)`);
    ...
    console.error("[BatchInsights] Error:", error);
    progress.status = 'error';
    progress.error = error instanceof Error ? error.message : 'Unknown error';
    onProgress?.(progress);
    throw error;
