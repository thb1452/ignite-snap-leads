-- Extend fn_map_markers_in_bounds with list filters for map/list sync
-- and ensure no hard cap below viewport demand (p_limit passed from client, default 60k)
DROP FUNCTION IF EXISTS public.fn_map_markers_in_bounds(numeric, numeric, numeric, numeric, text, text, text, integer, integer, integer);
...
BEGIN
  ...
  FROM properties p
  WHERE 
    -- Geographic bounds filter (viewport-based)
    ...
    AND p.latitude != 0
    AND p.longitude != 0
    -- Optional filters (match fn_properties_paged for list/map sync)
...
COMMENT ON FUNCTION public.fn_map_markers_in_bounds IS 'Fetches map markers within geographic bounds for viewport-based loading. Supports all list filters for sync. No fixed 1k cap - client passes p_limit (default 60k).';
