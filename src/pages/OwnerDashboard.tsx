...
function DashboardSections({ section, data, navigate }: { section: Section; data: Snapshot; navigate: (s: Section) => void }) {
  ...
  const failedFeeds = [data.requests, data.agents, data.outlets, data.uploads, data.reviews, data.sentToday, data.repliesToday, data.registry, data.research, data.tasks, data.taskReviews, data.mailboxSync, data.mailboxReview, data.mailboxTests, data.mailReviewHealth, data.mailboxSuggestions, data.collectionDeliveries, data.collectionProcessing, data.collectionOriginals, data.archivePlans, data.archiveVerifications, data.archiveCopies, data.archiveJobs, data.archiveControl, data.collectionEditorial, data.freshCollectionCount, data.customerAcceptedCollections, data.acquisitionControls, data.acquisitionPolicy, data.acquisitionAssignments, data.acquisitionHistory, data.acquisitionHealth, data.acquisitionCapacity, data.publicDownloadHealth].filter(f => f?.error).length;
