...
-- ============================================================================
-- EXPANDED NORMALIZATION: Add more mappings based on remaining unmapped types
-- ============================================================================
...
BEGIN
  ...
  -- ========== SAFETY ==========
  IF t ~ '(safety|hazard|danger|unsafe|egress|handrail|guardrail|railing|luminaire|wiring|circuit|carbon monoxide|health|attractive nuisance|nuisance affecting|rodent|vermin|sanitar|animal carcass|dead.*dying.*landscaping)' THEN
    RETURN 'Safety';
  END IF;
  ...
  -- ========== EXTERIOR ==========
  IF t ~ '(exterior|siding|paint|peeling|fascia|soffit|window|door|gutter|downspout|trim|corrosion|304\.[0-9]|protective treatment|weather tight|frame|screen|facade|ipmc 304|ipmc 308|cco |rubbish|garbage|accumulation)' THEN
    RETURN 'Exterior';
  END IF;
  ...
  -- ========== NAMES (inspector names in type field) ==========
  IF t ~ '^(john|matthew|dustin|kevin|austin|trevor|manuel|scott|pamella|nicholas|[a-z]+ [a-z]+)$' THEN
    RETURN 'Unknown';
  END IF;
