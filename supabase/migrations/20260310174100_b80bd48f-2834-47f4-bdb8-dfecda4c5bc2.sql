DROP FUNCTION IF EXISTS fn_properties_paged(integer,integer,text,text,text,integer,integer,integer,text,boolean,boolean,boolean);
DROP FUNCTION IF EXISTS fn_properties_by_category(text,text,text,text,integer,integer,integer,integer,integer,text,boolean,boolean,boolean);
    ...
    FROM (
      WITH ranked AS (
        SELECT 
          ...
          ROW_NUMBER() OVER (PARTITION BY p.state, p.city ORDER BY p.newest_violation_date DESC NULLS LAST) as city_rank
        FROM properties p
        WHERE (p_state IS NULL OR p.state ILIKE p_state)
          AND (p_city IS NULL OR p.city ILIKE p_city)
          AND (p_search IS NULL OR 
      ...
      FROM ranked
      ORDER BY city_rank ASC, newest_violation_date DESC NULLS LAST, id
      LIMIT p_page_size
      OFFSET v_offset
    ) props;
  ELSE
...
CREATE OR REPLACE FUNCTION fn_properties_by_category(
  p_category text,
  p_state text DEFAULT NULL,
  p_city text DEFAULT NULL,
  p_search text DEFAULT NULL,
