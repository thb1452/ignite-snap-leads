-- Fix lowercase state codes
UPDATE properties SET state = UPPER(state) WHERE state ~ '^[a-z]{2}$';

-- Recreate the materialized view with case-insensitive state matching
...
WHERE city IS NOT NULL 
  ...
  AND LENGTH(state) = 2
  AND UPPER(state) ~ '^[A-Z]{2}$'
  -- Allow: Title case, hyphenated, apostrophes
