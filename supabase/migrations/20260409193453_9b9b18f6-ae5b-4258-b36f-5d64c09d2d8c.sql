-- Fix fn_properties_paged to use index-friendly UPPER() instead of ILIKE for state/city
...
-- Also add an index on UPPER(state) to match the new query pattern
CREATE INDEX IF NOT EXISTS idx_properties_state_upper ON properties (UPPER(state));