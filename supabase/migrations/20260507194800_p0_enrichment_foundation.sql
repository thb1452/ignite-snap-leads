...
-- ============================================================================
-- Adds the canonical attribute store and queue infrastructure that future
-- enrichment agents (Atlas/Nova/etc.) will consume. Schema-only — no agent
-- code, no DB writes from this PR.
--
-- See docs/SNAP_IGNITE_MASTER_PLAN.md and PR #156 for context.
-- ============================================================================

-- ── parcel_attributes: canonical normalized attribute store ─────────────────
...
-- Mirror public.properties: shared-read across authenticated users.
-- Service-role writes only — no INSERT/UPDATE/DELETE policies for authenticated.
...
comment on table public.parcel_attributes is
  'P0 foundation: RLS mirrors public.properties (authenticated SELECT, USING(true)).
   County-entitlement gating will be applied to both parcel_attributes AND properties
   together in a follow-up PR once county_entitlements is captured in the repo.
   Do not tighten this table independently — keep parity with properties.';

-- ── enrichment_sources: registry of where attribute data is pulled from ─────
