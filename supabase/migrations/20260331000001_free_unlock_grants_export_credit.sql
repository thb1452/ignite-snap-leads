-- Bug 3 fix: free unlocks must also grant export access.
-- When fn_unlock_property consumes a free unlock, insert a +1 credit_ledger entry
-- (reason='free_unlock_export') so that export-csv's PAYG credit-balance gate allows
-- the user to export that property. Net cost is still $0 — it simply makes the user
-- eligible for the export-csv PAYG path without purchasing anything.
...
BEGIN
  ...
  IF v_free_remaining IS NOT NULL AND v_free_remaining > 0 THEN
    ...
    -- Grant 1 export credit so export-csv PAYG gate allows exporting this property.
    ...
    VALUES (p_user_id, 1, 'free_unlock_export', p_property_id,
            jsonb_build_object('property_id', p_property_id, 'note', 'free unlock export entitlement'));
