-- Subscribers see "credits" in the UI as remaining monthly export quota (useCreditBalance),
-- but fn_unlock_property only checked v_user_credits (PAYG credit_ledger). Ledger is often 0
-- for plan-only users, causing 402 insufficient_balance despite 148 showing in the header.
-- Consume 1 export slot from subscription_usage (same pool as CSV exports / Settings).
  ...
  IF v_plan_max IS NOT NULL THEN
    ...
    IF v_new_exports IS NOT NULL THEN
      ...
      RETURN jsonb_build_object(
        ...
        END,
        'credits_remaining', v_credit_balance
      );
    END IF;
  END IF;
