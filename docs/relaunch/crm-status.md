...
- **Manual outcome:** one atomic RPC records the outcome, completed action and replacement action (or explicit no follow-up). Outcomes do not send communications. Only `reached_owner` advances last-contacted time. Same request UUID + exact command is idempotent; changed commands and stale versions fail. The UI retains the exact uncertain command in per-tab, account-scoped recovery for reload/retry; a saved-receipt check can reconcile a committed result. After 24 hours it removes private draft text and retains only a blocking receipt pointer. Wrong-account, malformed and unavailable storage fail closed. Initial authenticated hydration preserves only the matching account’s validated recovery in sessionStorage; localStorage, signed-out hydration, account changes and permission failures purge it without renewing its TTL. Late completion callbacks cannot delete a newer account’s pending action. Do-not-contact persists at the lead and existing/future private-contact relationships; ordinary edits cannot clear it. Full activity notes remain while the restriction summary is bounded to 1,000 characters.
...
3. Complete the reload/retry journey in a real browser. Native PostgreSQL CI passed selected handoff/outcome contention scenarios across independent sessions at the commit recorded in [staging-verification.md](staging-verification.md). Recovery-storage tests also prove one activity after a committed outcome loses its acknowledgement and is replayed exactly. Hosted browser/auth integration remains a release check; later SQL/harness changes require a new native receipt.
