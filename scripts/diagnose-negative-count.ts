...
for (const job of jobs.slice(0, 10)) {
  ...
  console.log(`${status} ${job.id.slice(0, 8)}... | ${job.status.padEnd(12)} | Rows: ${(job.total_rows || 0).toString().padStart(6)} | Props: ${(job.properties_created || 0).toString().padStart(6)} | Viols: ${(job.violations_created || 0).toString().padStart(6)} | ${job.city}, ${job.state}`);
}
