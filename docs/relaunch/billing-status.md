...
The billing SQL/provider suite passed 30/30 tests after the mode guard change. The later paid-proration fix passed 32/32 targeted billing checks; `node --test tests/billing/reconciliation-review.test.mjs` passed six additional evidence-validation/read-only proposal tests. Root also ran the combined suite including these cases. Current real-provider execution results and limits are recorded in [sandbox status](stripe-sandbox-tests.md) and its linked sanitized receipt. A new isolated sandbox was approved; the earlier anonymous provisioning failure does not describe the later authenticated setup.

PGlite runs real PostgreSQL in one embedded connection. Concurrent JavaScript requests are queued there. The tests demonstrate idempotent replay and atomic rollback, **not a multi-backend lock-contention proof**. Provider requests can reach Stripe in a different order from ticket issuance; terminal states are explicitly protected, but nonterminal cross-process read ordering still needs staging validation. The separate native PostgreSQL CI now passes seven concurrency scenarios across three independent sessions; see [staging-verification.md](staging-verification.md) for its exact commit and artifact. Real Auth/PostgREST and gateway checks now pass in the separate [ephemeral local Supabase CI](local-supabase-ci.md); hosted migration and full hosted acceptance remain required. Provider methods in the unit suite remain stubs; see the separate sandbox report for provider evidence. The frontend typecheck passed after implementation.
