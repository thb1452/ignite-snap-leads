-- Add portal difficulty score to targets
ALTER TABLE public.targets
  ADD COLUMN IF NOT EXISTS portal_difficulty_score int CHECK (portal_difficulty_score BETWEEN 1 AND 5);

-- Add fulfillment quality columns to foia_requests
ALTER TABLE public.foia_requests
  ADD COLUMN IF NOT EXISTS data_quality_score int CHECK (data_quality_score BETWEEN 1 AND 5),
  ADD COLUMN IF NOT EXISTS fulfillment_file_url text,
  ADD COLUMN IF NOT EXISTS data_format text CHECK (data_format IN ('csv', 'pdf', 'image', 'mixed', 'other'));

-- Allow VAs to update portal_difficulty_score on targets they have been assigned
CREATE POLICY "va_update_portal_difficulty" ON public.targets
  FOR UPDATE TO authenticated
  USING (
    auth.uid() IS NOT NULL
    AND EXISTS (
      SELECT 1 FROM public.foia_assignments fa
      WHERE fa.target_id = targets.id
        AND fa.va_id = auth.uid()
    )
  )
  WITH CHECK (true);

-- Jurisdiction Intelligence Score (JIS) function
...
-- State-level response analytics
...
GRANT EXECUTE ON FUNCTION public.fn_jurisdiction_intelligence() TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_state_response_analytics() TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_fulfillment_overview() TO authenticated;
