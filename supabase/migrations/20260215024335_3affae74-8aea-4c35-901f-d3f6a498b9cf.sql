...
-- Add trial columns to user_subscriptions table
ALTER TABLE public.user_subscriptions
  ...
  ADD COLUMN IF NOT EXISTS trial_exports_used integer DEFAULT 0,
  ADD COLUMN IF NOT EXISTS trial_exports_limit integer DEFAULT 50;

-- Add constraint for trial_tier values
ALTER TABLE public.user_subscriptions
  ADD CONSTRAINT chk_trial_tier CHECK (trial_tier IS NULL OR trial_tier IN ('starter', 'professional', 'enterprise'));

-- Create a function to start a trial for a user
...
-- Create a function to increment trial exports
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'is_on_trial', v_sub.status = 'trial' AND v_sub.trial_ends_at > now(),
    ...
    'can_export', (v_sub.status = 'trial' AND v_sub.trial_ends_at > now() AND COALESCE(v_sub.trial_exports_used, 0) < COALESCE(v_sub.trial_exports_limit, 50)) OR v_sub.status IN ('active', 'trialing')
  );
END;
$$;
