...
function OwnerDashboardContent() {
  ...
  const query = useQuery({
    ...
    queryFn: () => loadOperations(user!.id),
    refetchInterval: autoRefresh ? 60000 : false,
    refetchIntervalInBackground: false, refetchOnWindowFocus: false,
    staleTime: 30000, gcTime: 0, retry: false,
  });
  const data = query.data;
  return <AppLayout><div className="mx-auto max-w-7xl space-y-6 px-4 py-6 md:px-8 md:py-8">
    ...
    <div className="rounded-xl border border-amber-200 bg-amber-50/70 p-4 text-sm text-amber-950 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-100">
      <p className="font-medium">Relaunch setup · Coverage still needs verification</p>
      <p className="mt-1">This view reads Snap’s connected database. The Azure worker feed, mailbox health, and publishing feed still need end-to-end verification. Empty feeds do not prove that everything is running.</p>
    ...
    <footer className="flex flex-wrap justify-between gap-3 border-t pt-4 text-xs text-muted-foreground">
      <span>Owner monitoring · No legacy VA signup required</span>
      <div className="flex gap-4"><Link className="underline" to="/admin-console">Data administration</Link><Link className="underline" to="/admin">Customer accounts</Link></div>
    </footer>
  </div></AppLayout>;
}
function DashboardSections({ section, data, navigate }: { section: Section; data: Snapshot; navigate: (s: Section) => void }) {
  ...
  const failedFeeds = [data.requests, data.agents, data.outlets, data.uploads, data.reviews, data.sentToday, data.repliesToday].filter(f => f.error).length;
  ...
  return <div className="space-y-6">
    {overview && <>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric label="Needs your review" value={data.reviews.error ? 'Unavailable' : number(data.reviews.total)} detail="Accessible items in the review feed" icon={<Clock3 className="h-4 w-4" />} />
    ...
    {section === 'Data quality' && <>
      <p className="text-sm text-muted-foreground">Your accessible uploads are shown below. Processed rows are not the same as approved, unique records. Agent deliveries and confidentiality checks need a connected quality feed.</p>
      <FeedPanel title="Your delivered files" feed={data.uploads} empty="No uploads are visible for your account. The worker database may hold deliveries that are not connected here yet.">
...
function Collection({ data, compact }: { data: Snapshot; compact: boolean }) {
  ...
  return <FeedPanel title="Collection queues" feed={data.requests} empty="No collection jobs are visible in this database. The old VA tracker is not being used as evidence of new agent activity.">
    {rows => {
  ...
  return <div className="space-y-5">
    ...
    <FeedPanel title="Registered news outlets" feed={data.outlets} empty="No outlets are visible in this connected database. Recover existing registrations before creating replacements.">
      {rows => <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{rows.map(row => <article key={row.id} className="rounded-xl border p-5">
        ...
        <p className="mt-3 text-xs text-muted-foreground">State coverage, mailbox health, and latest story: not connected.</p>
      </article>)}</div>}
    </FeedPanel>
    ...
    <Setup title="Publishing feed not connected" text="Story drafts, supporting records, editorial review, publication dates, and site health will appear here when the news sites are connected. No story counts or publishing activity have been assumed." />
  </div>;
}
